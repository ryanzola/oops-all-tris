...
export default class Canvas {
  constructor() {
    ...
    this.rz = this.resources.items.rz.scene.children[0]
  ...
  setGeometry() {
    ...
    // rz
    {
      this.rz.material = this.material3
      this.rz.castShadow = true

      this.geometry2 = this.rz.geometry.toNonIndexed()
      this.rz.geometry = this.geometry2.toNonIndexed()
      this.geometry2.toNonIndexed()
  
      let len2 = this.geometry2.attributes.position.count;
  
      let randoms2 = new Float32Array(len2)
      let centers2 = new Float32Array(len2 * 3)
      ...
      for(let i = 0; i < len2; i+=3) {
        ...
        let x = this.geometry2.attributes.position.array[i * 3 + 0]
        let y = this.geometry2.attributes.position.array[i * 3 + 1]
        let z = this.geometry2.attributes.position.array[i * 3 + 2]
  
        let x1 = this.geometry2.attributes.position.array[i * 3 + 3]
        let y1 = this.geometry2.attributes.position.array[i * 3 + 4]
        let z1 = this.geometry2.attributes.position.array[i * 3 + 5]
  
        let x2 = this.geometry2.attributes.position.array[i * 3 + 6]
        let y2 = this.geometry2.attributes.position.array[i * 3 + 7]
        let z2 = this.geometry2.attributes.position.array[i * 3 + 8]
        ...
        let center = new THREE.Vector3(x, y, z)
          ...
          centers2.set([center.x, center.y, center.z], i * 3)
          centers2.set([center.x, center.y, center.z], (i + 1) * 3)
          centers2.set([center.x, center.y, center.z], (i + 2) * 3)
      }

      this.geometry2.setAttribute('aRandom',new THREE.BufferAttribute(randoms2, 1))
      this.geometry2.setAttribute('aCenter',new THREE.BufferAttribute(centers2, 3))
    }
  }
  ...
  setMaterial() {
    // dancer
    {
      ...
      this.material2Obj = this.sheet.object('Material', {
    ...
    // rz
    {
      ...
      this.material3 = THREE.extendMaterial( THREE.MeshStandardMaterial, {
        ...
        vertex: {
          ...
          transformed.y += 10.0 * aRandom * (locprog);
          `
        },
      ...
      this.material3Obj = this.sheet.object('Material2', {
        progress: types.number(this.material3.uniforms.progress.value, { range: [0, 1] }),
      }).onValuesChange((values) => {
        this.material3.uniforms.progress.value = values.progress
      })
    }
  }
  ...
  setMesh() {
    ...
    // rz
    this.mesh2 = new THREE.Mesh(this.geometry2, this.material3)
    this.mesh2.customDepthMaterial = THREE.extendMaterial( THREE.MeshDepthMaterial, {
      template: this.material3
    });
    this.mesh2.castShadow = this.mesh2.receiveShadow = true
    // this.mesh2.rotation.y = Math.PI / 2
    this.mesh2.scale.multiplyScalar(0.4)
  ...
  update() {
    ...
    this.material3.uniforms.time.value = this.time.elapsed * 0.001
  }
}